...
class JoinState(GameState):
    ...
    def __new__(cls, gameStateMachine: StateMachine = None):
        if not cls.__instance:
            with lock:
                if not cls.__instance:
                    JoinState.__instance = super().__new__(cls)
    ...
    def completeJoinState(self) -> bool:
        self.gameStateMachine.setState(self.gameStateMachine.getPairState())
    ...
    async def join(self) -> bool:
        # do something
        await self.gameStateMachine.joinStateNotify()
        
        return True

    def add(self, userId: str, city: str, district: str, coordinate: list) -> bool:
        return self.gameStateMachine.addInPool(userId, city, district, coordinate)
