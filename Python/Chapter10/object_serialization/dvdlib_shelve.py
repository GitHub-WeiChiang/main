...
class DVD:
    ...
    def __repr__(self):
        return "DVD('{title}', {year}, {duration}, '{director}')".format(**vars(self))
...
class DvdDao:
    ...
    def all(self) -> List[object]:
        with shelve.open(self.dbname) as shelve_db:
            return [shelve_db[title] for title in sorted(shelve_db, key=str.lower)]

    def load(self, title: str) -> Optional[object]:
        with shelve.open(self.dbname) as shelve_db:
